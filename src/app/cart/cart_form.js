...
initMercadoPago("TEST-760407e9-e5a3-40bb-8aaa-af978b629476", {
  locale: "es-MX",
}); //'YOUR_PUBLIC_KEY')
...
export const CartForm = () => {
  ...
  const { cart, cart_subtotal, cart_descuento, cart_iva, cart_total } =
    useCartStore(); //debe de ir aqui arriba por ser un hook y evitar errores
  ...
  const mostrarMPbutton = () => {
    ...
    if (formaEntrega === "" || formaEntrega === undefined) {
      ...
      datos_entrega_nombre === "" ||
      datos_entrega_nombre === undefined
    ) {
      mostrarMensaje("Debes escribir el nombre en datos de entrega");
    } else if (
      datos_entrega_direccion === "" ||
      datos_entrega_direccion === undefined
    ) {
      mostrarMensaje("Debes escribir la direccion en datos de entrega");
    } else if (
      datos_entrega_correo === "" ||
      datos_entrega_correo === undefined
    ) {
      mostrarMensaje("Debes escribir el correo en datos de entrega");
    } else if (
      datos_entrega_telefono === "" ||
      datos_entrega_telefono === undefined
    ) {
              ...
              <div className="cart_pricing_table text-uppercase">
                ...
                <ul className="ul_li_block clearfix">
                  <li style={{ fontWeight: "800" }}>
                    <span>Subtotal</span>{" "}
                    <span>$ {cart_subtotal.toFixed(2)}</span>
                  </li>
                  <li style={{ fontWeight: "800" }}>
                    <span>Descuento</span>{" "}
                    <span>$ {cart_descuento.toFixed(2)}</span>
                  </li>
                  <li style={{ fontWeight: "800" }}>
                  ...
                  <li>
                    <div
                      className="btn_wrap pt-0 text-center"
                    ...
                    >
                      ...
                      {cart && cart.length > 0 && viewPayPalbutton === true ? (
                        <PayPalScriptProvider
                          options={{
                            clientId:
                              "AU0iYefwyGxxvHGKHsIsuOUgG90MvxKGnpJyko9VGWLmTwppetXKDT4NLETo60tu5g6WdzAwiIozvuzL",
                            currency: "MXN",
                          }}
                        >
                          <PayPalButtons
                            createOrder={async () => {
                              //vemos si hay existencias de todo
                              try {
                                let res = await clienteAxios.post(
                                  "/pedido/checkStock",
                                  {
                                    tipo_venta: "Tienda online",
                                    subtotal: cart_subtotal,
